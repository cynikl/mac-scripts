...
function check_is_duplicate_application {
    ...
    #echo "${item2add}"
    application_check1=`defaults read /Users/$homeown/Library/Preferences/com.apple.dock persistent-apps | grep -w _CFURLString | awk 'BEGIN { FS = "\"" } ; { print $4 }' | grep -x "${item2add}"`
    application_check2=`defaults read /Users/$homeown/Library/Preferences/com.apple.dock persistent-apps | grep -w _CFURLString | awk 'BEGIN { FS = "\"" } ; { print $4 }' | grep -x "${item2add}/"`
...
function check_is_duplicate_others {
    ...
    #echo "${item2add}"
    application_check1=`defaults read /Users/$homeown/Library/Preferences/com.apple.dock persistent-others | grep -w _CFURLString | awk 'BEGIN { FS = "\"" } ; { print $4 }' | grep -x "${item2add}"`
    application_check2=`defaults read /Users/$homeown/Library/Preferences/com.apple.dock persistent-others | grep -w _CFURLString | awk 'BEGIN { FS = "\"" } ; { print $4 }' | grep -x "${item2add}/"`
    if [ "${application_check1}" != "" ] || [ "${application_check2}" != "" ] ; then
...
function add_application {
    ...
    if [ -d "${item2add}" ] || [ -f "${item2add}" ] ; then
        ...
        defaults write /Users/$homeown/Library/Preferences/com.apple.dock persistent-apps -array-add "<dict><key>tile-data</key><dict><key>file-data</key><dict><key>_CFURLString</key><string>${item2add}</string><key>_CFURLStringType</key><integer>0</integer></dict></dict></dict>"        
    else
	...
	(( num_periods-- ))
        ...
        if [ -d "${item2add}" ] || [ ${num_periods} -eq 0 ]; then 
            #echo "Adding Item : ${item2add}"
             defaults write /Users/$homeown/Library/Preferences/com.apple.dock persistent-others -array-add "<dict><key>tile-data</key><dict><key>file-data</key><dict><key>_CFURLString</key><string>${item2add}</string><key>_CFURLStringType</key><integer>0</integer></dict></dict><key>tile-type</key><string>directory-tile</string></dict>"
        else
            #echo "Adding Item : ${item2add}"
            defaults write /Users/$homeown/Library/Preferences/com.apple.dock persistent-others -array-add "<dict><key>tile-data</key><dict><key>file-data</key><dict><key>_CFURLString</key><string>${item2add}</string><key>_CFURLStringType</key><integer>0</integer></dict></dict><key>tile-type</key><string>file-tile</string></dict>"
        fi      
    else
...
chown -R $homeown:staff $eachPath/Library/Preferences/com.apple.dock.plist
chmod 755 $eachPath/Library/Preferences/com.apple.dock.plist
